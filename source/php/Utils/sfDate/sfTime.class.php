...
class sfTime
{
	...
	public static function finalDayOfMonth($ts = null)
	{
		if(false){
			// BUG: jme- to fix
			// - if date is "2008-10-31" then final day of the month is "2008-11-30"
			// - this is a boundary issue
			// - TODO check the other boundaries. firstdayof month, week etc...
			//return sfTime::subtractDay(sfTime::firstDayOfMonth(sfTime::addMonth($ts)));
		}
		if( true ){
			// patch from thibault sebastien
			// jme- seems to work better but i think the bug is elsewhere (like in addMonth)
			// - *IF* the bug is actually here, all the finalBLABLA are bugged
			$nextMonthTs	= sfTime::addMonth($ts);
			$currentMonth	= date('m',$ts);
			$nextMonth	= date('m',$nextMonthTs);
			if($currentMonth + 1 < $nextMonth)	$nextMonthTs = sfTime::subtractMonth($nextMonthTs);
			return sfTime::subtractDay(sfTime::firstDayOfMonth($nextMonthTs));
		}
	}
